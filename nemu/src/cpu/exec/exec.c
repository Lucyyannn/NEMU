...
//   \not-356, \and-262, \or-357!, 
//   \xor-411!, \sal(shl)-383, \shr-383, 
//   \sar-383, \setcc-389!, \test-405
...
opcode_entry opcode_table [512] = {
  ...
  /* 0x20 */	IDEXW(mov_G2E,mov_c2r,4), EMPTY,IDEXW(mov_E2G,mov_r2c,4) , EMPTY,
...
//exec_real
make_EHelper(real) {
  uint32_t opcode = instr_fetch(eip, 1);//1 byte!
  decoding.opcode = opcode;//as opcode
  set_width(opcode_table[opcode].width);
  idex(eip, &opcode_table[opcode]);
}
